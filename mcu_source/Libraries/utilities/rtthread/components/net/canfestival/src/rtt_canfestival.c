...
{
    ACC[0] = sys.imu.cal_acc[0]*1000;
    ACC[1] = sys.imu.cal_acc[1]*1000;
    ACC[2] = sys.imu.cal_acc[2]*1000;
...
unsigned char canSend(CAN_PORT notused, Message *m)
{
    ...
    CAN_Send(HW_CAN1, m->cob_id, m->data, m->len);
    return 0;
}
...
void canopen_recv_thread_entry(void* parameter)
{
    ...
    while (1)
    {
        rt_mq_recv(cf_mq, &msg, sizeof(msg), RT_WAITING_FOREVER);
        _dump_msg("rev", &msg);
...
void canopen_system_init(void)
{
    ...
    /* CAN */
    SetPinMux(HW_GPIOA, 12, 0, 2, 3);   /* CAN_TX */
    SetPinMux(HW_GPIOA, 11, 0, 2, 0);   /* CAN_RX */
    ...
    cf_mq = rt_mq_create("cf_mq", sizeof(Message), 4, RT_IPC_FLAG_FIFO);
    ...
    tid = rt_thread_create("cf_rev", canopen_recv_thread_entry, RT_NULL, 1024, 28, 20);
...
void CAN1_RX1_IRQHandler(void)
{
    ...
    rt_mq_send(cf_mq, &m, sizeof(Message));
